    ...
    public void simulateTracking() {
        ...
        for (Shipment shipment : activeShipments) {
            ...
            try {
                ...
                int riskScore = 0;
                if (aiResponse != null && aiResponse.containsKey("risk_score")) {
                    Object riskObj = aiResponse.get("risk_score");
                    if (riskObj instanceof Number) {
                        riskScore = ((Number) riskObj).intValue();
                    } else {
                        try {
                            riskScore = (int) Math.round(Double.parseDouble(riskObj.toString()));
                        } catch (Exception e) {
                            riskScore = 0;
                        }
                    }
                ...
                if (riskScore > 75) {
                    ...
                    if (newRoute != null && newRoute.containsKey("polyline")) {
                        ...
                        if (newRoute.containsKey("alerts") && newRoute.get("alerts") instanceof List) {
                            List<?> rawAlerts = (List<?>) newRoute.get("alerts");
                            for (Object alertObj : rawAlerts) {
                                if (alertObj != null) alerts.add(alertObj.toString());
                            }
                        }
