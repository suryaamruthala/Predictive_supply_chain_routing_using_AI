...
const roleColor = (role) => {
    if (!role) return 'text-gray-400 bg-gray-500/10 border-gray-500/30';
    ...
    const fetchUsers = async () => {
        try {
            ...
            setUsers([
                ...
                { id: 2, username: 'fleet_cmd_01', role: 'OPERATOR' },
                { id: 3, username: 'logistics_mgr', role: 'OPERATOR' },
            ]);
        } finally {
            setLoading(false);
        }
                        ...
                        {loading ? (
                            ...
                            <tr key={user.id} className="hover:bg-white/5 transition-colors">
                                ...
                                <td className="py-3 px-4">
                                    <span className={`text-xs font-bold px-2 py-1 rounded border flex items-center gap-1 w-fit ${roleColor(user.role)}`}>
